...
export default function EventDetail() {
  ...
  if (notFoundFlag || !event) return <div className="text-center py-12 opacity-60">Event not found.</div>;
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <div className="md:w-80 flex-shrink-0">
          <img
            ...
            alt={event.name}
            onError={e => { (e.currentTarget as HTMLImageElement).src = '/placeholders/default.jpg'; }}
        ...
        <div className="flex-1 min-w-0">
          ...
          {event.tagline && (
            <p className="text-base font-medium opacity-75 mb-3">{event.tagline}</p>
          )}
          ...
          {event.organizer && (
            <p className="text-sm italic opacity-70 mb-4">Organized by: {event.organizer}</p>
          )}

          {event.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {event.tags.map(t => (
                <span key={t} className="px-2 py-0.5 rounded-full text-xs bg-gray-100 dark:bg-gray-700 opacity-80">
                  {t}
                </span>
              ))}
            </div>
