...
const EMPTY = {
  ...
  location: '', department: '', speakerBio: '', video: '', tagsRaw: '', published: true,
};
...
export default function AdminColloquium() {
  ...
  const resetForm = () => { setForm({ ...EMPTY }); setEditId(null); setThumbFile(null); setShowForm(false); setError(''); };

  const startEdit = (c: Colloquium) => {
    setForm({
      name: c.name, speaker: c.speaker || '', abstract: c.abstract,
      date: c.date, time: c.time || '', location: c.location || '',
      department: c.department || '', speakerBio: c.speakerBio || '',
      video: c.video || '', tagsRaw: (c.tags || []).join(', '),
  ...
  return (
    <div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white dark:bg-[#25293c] border border-gray-200 dark:border-gray-700 rounded-xl p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field label="Title *"><input required value={form.name} onChange={e => set('name', e.target.value)} className={iCls} placeholder="Talk title" /></Field>
            <Field label="Speaker *"><input required value={form.speaker} onChange={e => set('speaker', e.target.value)} className={iCls} placeholder="Dr. Name" /></Field>
            <Field label="Date *"><input required type="date" value={form.date} onChange={e => set('date', e.target.value)} className={iCls} /></Field>
            <Field label="Time"><input type="time" value={form.time} onChange={e => set('time', e.target.value)} className={iCls} /></Field>
            <Field label="Location *"><input required value={form.location} onChange={e => set('location', e.target.value)} className={iCls} placeholder="Room / venue" /></Field>
            <Field label="Department"><input value={form.department} onChange={e => set('department', e.target.value)} className={iCls} placeholder="Affiliation" /></Field>
            <Field label="Video URL (YouTube embed)"><input type="url" value={form.video} onChange={e => set('video', e.target.value)} className={iCls} placeholder="https://www.youtube.com/embed/..." /></Field>
            <Field label="Tags (comma-separated)"><input value={form.tagsRaw} onChange={e => set('tagsRaw', e.target.value)} className={iCls} placeholder="physics, astrophysics" /></Field>
            <Field label="Thumbnail Image"><input type="file" accept="image/*" onChange={e => setThumbFile(e.target.files?.[0] || null)} className={iCls} /></Field>
            <Field label="Abstract *" className="md:col-span-2"><textarea required rows={4} value={form.abstract} onChange={e => set('abstract', e.target.value)} className={iCls} placeholder="Describe the talk..." /></Field>
            <Field label="Speaker Bio" className="md:col-span-2"><textarea rows={3} value={form.speakerBio} onChange={e => set('speakerBio', e.target.value)} className={iCls} placeholder="Brief bio of the speaker..." /></Field>
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-10 opacity-60 bg-white dark:bg-[#25293c] rounded-xl border border-gray-200 dark:border-gray-700">No colloquium entries yet.</div>
      ) : (
          ...
          {entries.map(c => (
            <div key={c.id} className="bg-white dark:bg-[#25293c] border border-gray-200 dark:border-gray-700 rounded-xl p-4 flex items-center gap-4 flex-wrap">
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs opacity-60 mt-0.5">{c.date}{c.speaker ? ` · ${c.speaker}` : ''} {!c.published && '· Draft'}</div>
              </div>
              <div className="flex gap-2 flex-shrink-0">
                <button onClick={() => startEdit(c)} className="px-3 py-1.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Edit</button>
                {canDel && (
                  <button onClick={() => handleDelete(c.id)} className="px-3 py-1.5 text-sm rounded-lg border border-red-300 dark:border-red-700 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">Delete</button>
                )}
              </div>
            </div>
          ))}
